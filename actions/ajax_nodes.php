...
require_once('classes/module_base.class.php');
        ...
        $secondary_id      = optional_param('secondary_id', null, PARAM_INT);
        ...
        if (empty($modulesettings)) {
            block_ajax_marking_update_modules(false);
            $modulesettings = unserialize(get_config('block_ajax_marking', 'modules'));
        }
...
//        $this->students->ids = new stdClass;
        ...
        $viewsneedingcourses = array(
                'html',
                'main',
                'config_course',
                'config_main_tree',
                'course');
        $needslistofcourses = in_array($this->type, $viewsneedingcourses);

        $modulenodetype = in_array($this->type, array_keys($this->modulesettings));
        ...
        if ($needslistofcourses || $modulenodetype || $level_check) {
        ...
        if ($standard_groups_type || $modulenodetype || $level_check) {
                ...
                foreach ($this->courses as $course) {
                    ...
                    foreach ($this->modulesettings as $modulename => $module) {
                        ...
                        if (in_array($modulename, $enabledmods)) {
                            $count += $this->$modulename->count_course_submissions($courseid, $studentids);
                        }
                    }
...
$ajaxresponse = new ajax_marking_response;