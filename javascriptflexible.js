...
// Building the new refactored infrastructure here by gradually moving things

/**
 *Make a base class that can be extended to create the main and congig trees
 */
YAHOO.ajax_marking_block.tree_base = function (tree_div, icon_div, status_div) {

    YAHOO.ajax_marking_block.tree_base.superclass.constructor.call(tree_div);

    this.icon    = document.getElementById(icon_div);
    this.div     = document.getElementById(status_div);
}

YAHOO.lang.extend(YAHOO.ajax_marking_block.tree_base, YAHOO.widget.TreeView);

//YAHOO.ajax_marking_block.tree_base.prototype.
    ...
    YAHOO.ajax_marking_block.nodeHolder = '';
    ...
    // what does this do?
    YAHOO.ajax_marking_block.checkVar = 6;
    ...
    YAHOO.ajax_marking_block.windowobj = '';
    ...
    YAHOO.ajax_marking_block.frameTimerVar = '';

    YAHOO.ajax_marking_block.t = 0;
        ...
        if (responseArray != null) {
            ...
            switch (type) {
                ...
                case 'main':
                    ...
                    YAMB.build_course_nodes(responseArray, YAMB.main);
                ...
                case 'course':
                    ...
                    YAMB.build_assessment_nodes(responseArray, YAMB.main);
                ...
                case 'groups':
                    ...
                    YAMB.build_group_nodes(responseArray, YAMB.main);
                    break;

                case 'submissions':

                    YAMB.build_submission_nodes(responseArray, YAMB.main);
                    break;

                case 'config_main':

                    YAMB.build_course_nodes(responseArray, YAMB.config);
                    break;

                case 'config_course':

                    YAMB.build_assessment_nodes(responseArray, YAMB.config);
                    break;
                ...
                case 'config_groups':
                    ...
                    YAMB.make_config_groups_list(responseArray, YAMB.config);
                    break;
                ...
                case 'config_set':
                    ...
                    if (responseArray[0].value === false) {
                        ...
                        YAMB.remove_all_child_nodes(YAMB.config.status);
                        YAMB.config.status.appendChild(label);
                    } else {
                ...
                case 'config_group_save':
                    ...
                    if (responseArray[0].value === false) {
                        YAMB.config.status.innerHTML = 'AJAX error';
                    } else {
                        YAMB.enable_config_radio_buttons();
                    }
     ...
     */
    YAHOO.ajax_marking_block.build_course_nodes = function(nodesArray, AJAXtree) {
     ...
     */
    YAHOO.ajax_marking_block.request_node_data = function(node, onCompleteCallback) {
     ...
     * accurate
     */
    YAHOO.ajax_marking_block.update_parent_node = function(AJAXtree, node) {
     ...
     */
    YAHOO.ajax_marking_block.build_assessment_nodes = function(nodesArray, AJAXtree) {
    ...
    */
    YAHOO.ajax_marking_block.build_submission_nodes = function(nodesArray, AJAXtree) {
     ...
     */
    YAHOO.ajax_marking_block.build_group_nodes = function(responseArray, AJAXtree) {
    ...
    YAHOO.ajax_marking_block.refresh_tree_after_changes = function(tree, frames) {
        tree.root.refresh();
    ...
    */
    YAHOO.ajax_marking_block.update_total_count = function() {
    ...
    *
    */
    YAHOO.ajax_marking_block.remove_node_from_tree = function(windowUrl, AJAXtree, nodeUniqueId, frames) {
    ...
    */
    YAHOO.ajax_marking_block.refresh_tree = function(treeObj) {
     ...
     */
    YAHOO.ajax_marking_block.update_node_count = function (newNode, newCount) {
     ...
     */
    YAHOO.ajax_marking_block.alter_workshop_popup = function (me, parent, course) {
        var els ='';
     ...
     * confirmation page
     */
    YAHOO.ajax_marking_block.alter_forum_popup = function (me) {
        var els ='';
        var name = navigator.appName;
     ...
     */
    YAHOO.ajax_marking_block.alter_quiz_popup = function (me) {
        var els = '';
        var lastButOne = '';
...
/*
    ...
    YAHOO.ajax_marking_block.build_config_overlay = function() {
        ...
        if (!YAHOO.ajax_marking_block.greyOut) {
            ...
            YAMB.greyOut.beforeHideEvent.subscribe(function() {
                YAMB.refresh_tree(YAMB.main);
            });

            YAMB.greyOut.render(document.body);
            YAMB.greyOut.show();
            // Now that the grey overlay is in place with all the divs ready, we build the config tree
            YAMB.config = new YAMB.AJAXtree('configTree', 'configIcon', 'configStatus', true);
            YAMB.build_ajax_tree(YAMB.config);

            YAMB.icon.setAttribute('class', 'loaderimage');
            YAMB.config.icon.setAttribute('className', 'loaderimage');
    ...
    /**
     * This function is to sub-class the AJAXtree function in order to create the
     * main trees.
     */
    YAHOO.ajax_marking_block.tree_factory = function (type) {
        ...
        switch (type) {
            case 'main':
                treeObject         = new YAHOO.widget.TreeView('treediv');
                treeObject.icon    = document.getElementById('mainIcon');
                treeObvject.div    = document.getElementById('status');
            ...
            case 'config':
                ...
                treeObject         = new YAHOO.widget.TreeView('configTree');
        ...
        // can probably get rid of this nad just use getRoot() when needed.
   ...
   /**
     ...
     */
    YAHOO.ajax_marking_block.build_ajax_tree = function(tree) {
        ...
        if (tree.loadCounter === 0) {
            ...
            // as this check was to fix an old bug
            ...
            if (tree.config) {
                postData = 'id='+amVariables.userid+'&type=config_main&userid=';
                postData += amVariables.userid;
            } else {
                postData = 'id='+amVariables.userid+'&type=main&userid=';
                postData += amVariables.userid;
            }
